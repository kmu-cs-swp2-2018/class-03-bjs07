...
class HangmanGame(QWidget):
    ...
    def guessClicked(self):
        ...
        # 입력이 영문자인지 판단하고, 아닌 경우 메시지 출력, 리턴
        if not guessedChar.isalpha():
            self.message.setText(guessedChar + ' is not alphabet!')
            return
        ...
        if success == False:
            ...
            # 메시지 출력
            self.message.setText(guessedChar + ' is not in secretWord')
            pass
        ...
        # 메시지 ("Fail!" + 비밀 단어) 출력하고, self.gameOver 는 True 로
            ...
            self.message.setText('Fail! ' + self.guess.displaySecretWord())
            self.gameOver = True
